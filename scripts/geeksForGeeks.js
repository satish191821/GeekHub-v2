...
function showExtensionContextWarning() {
  ...
  const existingToast = document.getElementById("gfg-to-github-context-toast");
  if (existingToast) {
        ...
        try {
          ...
          safeStorageGet("userStatistics", (statistics) => {
            ...
            if (uploadFileName === "README.md" && sha === null) {
              ...
              userStatistics.basic += normalizedDifficulty === "Basic" ? 1 : 0;
      ...
      const targets = [
        {
          ...
          encodedContent: btoa(unescape(encodeURIComponent(problemStatement))),
        },
      ];
...
const loader = setInterval(() => {
  ...
  if (
    ...
    if (!gfgSubmitButton || gfgSubmitButton.dataset.gfgToGithubListener) return;
    ...
    gfgSubmitButton.addEventListener("click", function () {
      ...
      const submissionLoader = setInterval(() => {
        ...
        if (
          ...
          const difficultyBadgeHTML = getDifficultyBadgeHTML(problemDifficulty);
